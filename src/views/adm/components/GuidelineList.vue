<template>
        <Container>
            ...
            <div class="relative overflow-x-auto shadow-md bg-white sm:rounded-lg" v-else>
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 ">
                    ...
                    <tbody>
                        <tr v-if="filteredGuidelines.length > 0" class="bg-white border-b   hover:bg-gray-50 "
                            v-for="(guideline, index) in filteredGuidelines" :key="guideline.id">
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                {{ guideline.numeration }}
                            </th>
                            <td class="px-6 py-4">
                            ...
                            <td class="flex gap-2 px-6 py-4 text-right justify-between items-center ">
                                <p class="font-medium text-blue-600  hover:cursor-pointer hover:underline"
                                    ...
                                    Editar</p>
                                <p @click="deleleGuideline(guideline)">
        ...
        <ModalCardComponent>
            <CardComponent>
                <Title text="Atualizar Diretriz" />
            </CardComponent>
            <form action="#" @submit.prevent="updateGuideline">
                <div class="grid gap-4 mb-4 sm:grid-cols-2 sm:gap-6 sm:mb-5">
                    <div class="sm:col-span-2">
                        ...
                        <textarea type="text" name="description" id="description"
                            ...
                            v-model="editGuideline.description" placeholder="Descrição"/>
                        <p class="px-2 mt-2 text-xs text-red-600 " v-if="v$.description.$error">
                            {{ "Nome obrigatório!" }}</p>
                    </div>
                    <div class="w-full">
                        <label for="numeration" class="block mb-2 text-sm font-medium text-gray-900">Número Diretriz</label>
                        <input type="number" name="numeration" id="numeration"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                            v-model="editGuideline.numeration" placeholder="Objetivo">
...
const editGuideline = ref<Guideline>({
    id: 0,
    description: '',
    numeration:0,
})
...
const v$ = useVuelidate(rules, editGuideline)


const filteredGuidelines = computed(() => {
    let guidelines = dataGuidelines.value


    if (searchDescription.value !== '') {
        guidelines = guidelines.filter(item => item.description.toLowerCase().includes(searchDescription.value.toLowerCase()))
    }
...
const updateGuideline = async () => {
    ...
    if (result) {
        await axiosInstance.put(`/api/guidelines/${editGuideline.value.id}`,
            {
                description: editGuideline.value.description,
                numeration: editGuideline.value.numeration,
...
const deleleGuideline = async (data: any) => {

    Swal.fire({
        title: "Você tem certeza?",
        text: "Você perderá todas informações relacionadas a esta Diretriz e os objetivos!",
        icon: "warning",
        ...
        if (result.isConfirmed) {
            axiosInstance.delete(`/api/guidelines/${data.id}`)
                .then(response => {
                    Swal.fire({
                        title: "Deleted!",
                    ...
                    dataGuidelines.value = dataGuidelines.value.filter(item=> item.id !== data.id)
                })
                .catch(error => {
                    Swal.fire({
                        title: "Eroor!",
...
const { dataGuidelines, isload } = useGuidelinesList()
